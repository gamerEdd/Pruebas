...
class BuySpecialistAI(IndicatorBase):
    """IA especializada EXCLUSIVAMENTE en detectar oportunidades de COMPRA"""
    ...
    def __init__(self, log_callback=None, dataset_manager=None, timeframe='M1'):
        ...
        # Pesos SIMÉTRICOS — idénticos a SELL specialist (mismo potencial máximo)
        self.weights = {
            'rsi_oversold': 0.17,
            'support_bounce': 0.15,
            'bullish_momentum': 0.10,
            'macd_bullish': 0.05,
            'volume_confirmation': 0.16,
            'bollinger_lower': 0.05,
            'stochastic_oversold': 0.06,
            'cci_bullish': 0.12,
            'williams_r_oversold': 0.12,
        ...
        self.min_recovery_potential = 0.5  # 0.5% mínimo
        ...
        self.oversold_condition_history = []    # [True/False, ...] de cuándo RSI < 30
        
        # ⭐ NUEVO (CALIBRATION FIX): Velocity Detector para cambios rápidos
        self.last_analysis_time = time.time()
        self.last_rsi = 50
        self.last_volume_sum = 0
        self.direction_velocity_bonus = 0  # +0 a +35 según velocidad de cambio
        self.has_rapid_change = False      # Detecta si hubo cambio > 3.0/s
    ...
    def _apply_session_context_bias(self, final_score, confidence):
        ...
        try:
            ...
            wr_edge = (buy_wr - sell_wr) / 100.0
            profit_edge = np.tanh((buy_p - sell_p) / 10.0)
            streak_penalty = min(0.25, buy_ls * 0.08)
            opp_boost = min(0.12, sell_ls * 0.04)
    ...
    def _update_live_signal_cache(self, recommendation, confidence, score):
        """Guarda la señal rápida del BUY specialist para trigger de apertura."""
        try:
    ...
    def get_live_open_signal(self, conf_threshold=None, max_age_sec=1.5):
        ...
        try:
            ...
            thr = float(self.thresholds.get('confidence_high', 65)) if conf_threshold is None else float(conf_threshold)
            rec = str(sig.get('recommendation', 'HOLD')).upper()
            conf = float(sig.get('confidence', 0.0) or 0.0)

            if rec == 'BUY' and conf >= thr:
    ...
    def train(self, snapshots):
        ...
        try:
            ...
            self.log(
                f"[TRAIN] BUY Specialist entrenado con {len(snaps)} barras. "
            ...
            self.log(f"Error entrenando BUY Specialist: {e}", 'error')
    ...
    def _update_dataset_insights(self):
        ...
        try:
            ...
            if insights:
                ...
                buy_cond = insights.get('best_buy_conditions', {})
                if buy_cond.get('requires_rsi_oversold'):
                    self.thresholds['rsi_oversold'] = 35  # Más estricto
                
                self.log("[EMOJI] Insights del dataset aplicados a BUY Specialist", 'info')
        except Exception as e:
            self.log(f"[ADVERTENCIA] Error actualizando insights: {e}", 'warning')
    
    # ⭐ NUEVO (P0 Phase 7): Análisis de volatilidad dinámica
    def _detect_crosses(self, features):
        """
        ⭐ P1 PHASE: Detecta NUEVOS cruces de osciladores
        
        Distingue entre:
        - Oscilador ACABA de cruzar umbral (fresh signal)
        - Oscilador LLEVA TIEMPO en condición (stale signal)
        ...
        try:
            ...
            # Detectar cruce Williams %R (BUY: de -80+ → <-80)
            current_williams = features.get('williams_r', -50)
            if self.previous_williams_r is not None:
                if self.previous_williams_r >= -80 and current_williams < -80:
    ...
    def _requires_confluent_entry(self, cross_strength, divergences):
        ...
        try:
            ...
            bullish_div = divergences.get('bullish_divergence', False)
            ...
            elif cross_strength == 2 and bullish_div:
                reason = f"[INFO] 2/4 cruces + Divergencia Alcista ({temporal_status})"
            else:
    ...
    def _calculate_divergences(self, current_features, current_price):
        ...
        try:
            divergences = {
                'bullish_divergence': False,        # Alcista (buena)
                'bearish_divergence': False,        # Bajista (mala)
                'hidden_bearish_divergence': False, # Oculta bajista
                'divergence_strength': 0.0,         # Qué tan fuerte
                'penalty': 1.0,                     # Multiplicador a aplicar al score
            }
            
            if self.previous_price_high is None or self.previous_rsi is None:
                return divergences  # Sin datos históricos
            
            current_rsi = current_features.get('rsi', 50)
            
            # 📉 DIVERGENCIA BAJISTA (mala para BUY) 🚫
            # Precio sube pero RSI baja = vendedores ganando
            if current_price > self.previous_price_high and current_rsi < self.previous_rsi:
                ...
                divergences['penalty'] = 0.4  # Penalizar -60% por divergencia bajista
                self.log(
                    f"[DIVERGENCIA] BAJISTA DETECTADA: Precio ↑{current_price:.2f} vs RSI ↓{current_rsi:.1f} "
                    f"(fuerza: {strength:.1f}%) → Penalizar BUY",
                    'warning'
                )
            
            # 📈 DIVERGENCIA ALCISTA (buena para BUY) ✅
            # Precio baja pero RSI sube = compradores ganando
            elif current_price < self.previous_price_low and current_rsi > self.previous_rsi:
                divergences['bullish_divergence'] = True
                strength = abs(current_rsi - self.previous_rsi) / self.previous_rsi * 100
                divergences['divergence_strength'] = strength
                divergences['penalty'] = 1.3  # Bonus +30% por divergencia alcista
                self.log(
                    f"[DIVERGENCIA] ALCISTA DETECTADA: Precio ↓{current_price:.2f} vs RSI ↑{current_rsi:.1f} "
                    f"(fuerza: {strength:.1f}%) → Bonus BUY",
                    'success'
                )
            
            # 📊 DIVERGENCIA OCULTA BAJISTA (continuación bajista)
            # Precio baja y RSI baja = bajista continúa
            elif current_price < self.previous_price_low and current_rsi < self.previous_rsi:
                divergences['hidden_bearish_divergence'] = True
                strength = abs(current_rsi - self.previous_rsi) / self.previous_rsi * 100
                divergences['divergence_strength'] = strength
                divergences['penalty'] = 0.7  # Penalizar -30% (menos que divergencia directa)
                self.log(
                    f"[DIVERGENCIA] OCULTA BAJISTA: Precio ↓ y RSI ↓ → Continuación bajista, evitar BUY",
                    'warning'
                )
    ...
    def _validate_signal_quality(self, final_score, crosses, divergences):
        ...
        try:
            ...
            if divergence_penalty < 1.0:
                self.log(f"[QUALITY] Divergencia bajista penaliza score ({final_score:.1f} × {divergence_penalty} = {adjusted_score:.1f})", 'warning')
            elif divergence_penalty > 1.0:
                self.log(f"[QUALITY] Divergencia alcista bonus score ({final_score:.1f} × {divergence_penalty} = {adjusted_score:.1f})", 'success')
    ...
    def analyze(self, symbol, market_snapshots=None, check_recovery_potential=True, min_recovery_pct=0.5):
        """
        Analiza el mercado SOLO para oportunidades de COMPRA
        ...
        try:
            ...
            # Preferir datos pasados via market_snapshots si se proporcionan
                ...
                # Obtener datos históricos del último minuto desde MT5
            ...
            # Calcular indicadores enfocados en COMPRA (ampliados)
            features = {
                'rsi': self._calculate_rsi(close),
                'support_level': self._find_support(low, close[-1]),
                'momentum': self._calculate_momentum(close),
                'macd_signal': self._calculate_macd_bullish(close),
                'volume_trend': self._calculate_volume_trend(volume),
                'bollinger_position': self._calculate_bollinger_position(close),
                'trend_strength': self._calculate_trend_strength(close, 'bullish'),
            ...
            # Rastrear condición de sobreventa (RSI < 30)
            rsi_oversold = features['rsi'] < 30
            self.oversold_condition_history.append(rsi_oversold)
            self.oversold_condition_history = self.oversold_condition_history[-max_history:]
            
            # ⭐ Calcular frescura de condición de sobreventa
            rsi_freshness = self._calculate_signal_freshness(
                self.oversold_condition_history,
                lambda x: x == True,  # Condición: está en sobreventa
                max_lookback=max_history
            )
            
            features['rsi_freshness'] = rsi_freshness['freshness_score']
            features['rsi_candles_oversold'] = rsi_freshness['candles_in_condition']
            
            # Log de frescura
            if rsi_freshness['candles_in_condition'] > 0:
                self.log(
                    f"[FRESHNESS] RSI en sobreventa {rsi_freshness['candles_in_condition']} velas "
            ...
            scores = self._calculate_buy_scores(features, volatility_factor)
            ...
            # Si RSI está muy bajo (muy oversold) → agregar +30 puntos
            if features['rsi'] < 25:
                final_score += 30
                self.log(f"[BONUS] RSI muy bajo ({features['rsi']:.1f}) → +30 puntos", 'success')
            
            # Si MACD está en cruza alcista → agregar +25 puntos
            macd_signal_val = features.get('macd_signal', 0)
            if isinstance(macd_signal_val, (int, float)) and macd_signal_val > 0:
                final_score += 25
                self.log(f"[BONUS] MACD cruza alcista ({macd_signal_val:.4f}) → +25 puntos", 'success')
            ...
            self.log(
                ...
                f"Divergencias: {'Alcista ✅' if divergences.get('bullish_divergence') else ('Bajista 🚫' if divergences.get('bearish_divergence') else 'Normal')}",
                'info'
            )
            ...
            # Reversión rápida alcista: evita que BUY quede clavado en 0 durante giros fuertes.
            if rsi_velocity > 0.8 and features.get('momentum', 0) > 0:
                ...
                self.log(
                    f"[REBOUND_BUY] RSI vel {rsi_velocity:.2f}/s + momentum {features.get('momentum', 0):.2f} -> score {prev_score:.1f}->{final_score:.1f}",
                    'success'
                )
            ...
            support = features.get('support_level', 999)
            ...
            # Penalizar si momentum negativo — proporcional a magnitud
            if momentum < -0.5:
                final_score = final_score * 0.50  # -50%: caída muy intensa
                self.log(f"[MOMENTUM_ANALYSIS] Momentum fuerte negativo ({momentum:.2f}) → BUY score -50%", 'warning')
            elif momentum < -0.25:
                final_score = final_score * 0.65  # -35%: caída moderada
                self.log(f"[MOMENTUM_ANALYSIS] Momentum moderado negativo ({momentum:.2f}) → BUY score -35%", 'info')
            elif momentum < 0:
                final_score = final_score * 0.80  # -20%: caída leve
                self.log(f"[MOMENTUM_ANALYSIS] Momentum leve negativo ({momentum:.2f}) → BUY score -20%", 'info')
            
            # Penalizar si soporte muy lejano
            if support > 0.15:
                final_score = final_score * 0.8  # -20% score
                self.log(f"[SUPPORT_ANALYSIS] Soporte lejano ({support:.2f}%) → score -20%", 'info')
            ...
            if scores.get('rsi_oversold', 0) >= 40:
                main_indicators_count += 1
            if scores.get('support_bounce', 0) >= 40:
                main_indicators_count += 1
            if scores.get('bullish_momentum', 0) >= 40:
                main_indicators_count += 1
            if scores.get('macd_bullish', 0) >= 40:
                main_indicators_count += 1
            ...
            # ⭐ AJUSTE POR VELAS CONSECUTIVAS (activo desde 2 velas, simétrico con SELL)
            if len(close) >= 2:
                ...
                for _ci in range(len(close)-1, max(0, len(close)-9), -1):
                    if (_cn_dir == 1 and close[_ci] >= close[_ci-1]) or (_cn_dir == -1 and close[_ci] < close[_ci-1]):
                        ...
                        break
                # Velas CONTRA BUY (bajistas)
                if _cn_dir == -1 and _cn_count >= 6:
                    final_score = final_score * 0.35
                    confidence = max(0, confidence * 0.60)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas bajistas → BUY -65%, conf -40%", 'warning')
                elif _cn_dir == -1 and _cn_count >= 4:
                    final_score = final_score * 0.55
                    confidence = max(0, confidence * 0.75)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas bajistas → BUY -45%, conf -25%", 'warning')
                elif _cn_dir == -1 and _cn_count >= 2:
                    final_score = final_score * 0.75
                    confidence = max(0, confidence * 0.88)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas bajistas → BUY -25%, conf -12%", 'warning')
                # Velas A FAVOR de BUY (alcistas)
                elif _cn_dir == 1 and _cn_count >= 6:
                    final_score = min(100, final_score * 1.35)
                    confidence = min(100, confidence + 15)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas alcistas → BUY +35%, conf +15", 'success')
                elif _cn_dir == 1 and _cn_count >= 4:
                    final_score = min(100, final_score * 1.20)
                    confidence = min(100, confidence + 10)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas alcistas → BUY +20%, conf +10", 'success')
                elif _cn_dir == 1 and _cn_count >= 2:
                    final_score = min(100, final_score * 1.10)
                    confidence = min(100, confidence + 5)
                    self.log(f"[CANDLE_TREND] {_cn_count} velas alcistas → BUY +10%, conf +5", 'success')
            ...
            self.log(f"[SESSION] BUY {session_detail}", 'info')
            recommendation = self._make_buy_recommendation(final_score, confidence, features)
            ...
            if check_recovery_potential:
                ...
                if recovery_potential < min_recovery_pct:
                    self.log(
                        ...
                        f"→ RECHAZADA operación BUY",
                        'warning'
                    )
                    recommendation = 'HOLD'
                    final_score = final_score * 0.5  # Penalizar score
            ...
            result = {
                ...
                'cross_strength': crosses.get('cross_strength', 0),
                'divergence_type': 'bullish' if divergences.get('bullish_divergence') else ('bearish' if divergences.get('bearish_divergence') else 'none'),
            ...
            self.log(
                f"[OK] {self.name} | Score: {final_score:.1f} | Conf: {confidence}% | "
    ...
    # ⭐ NUEVO: Calcular potencial de recuperación
    def _calculate_recovery_potential(self, symbol, features):
        """
        SISTEMA SIMETRICO BUY/SELL: Calcula potencial de recuperación
        BUY: Usa máx 24h como destino, mín como origen
        SELL: Al revés (max como origen, min como destino)
        """
        try:
            if self.market_history_24h is None:
                return 100.0
            ...
            # BUY: Ganancia potencial vs Distancia recorrida
            # Si precio está cerca del mín 24h → entrada buena
            # Si techo 24h está lejano → salida con ganancia posible
            distance_from_min = ((current - lowest) / lowest * 100) if lowest > 0 else 0
            distance_to_max = ((highest - current) / current * 100) if current > 0 else 0
            
            # Potencial = Ganancia posible / Distancia ya corrida
            # BUY alto cuando: precio bajo (cerca mín) + techo lejano
            if distance_from_min <= 0.1:
                return 100.0  # Precio EN el mínimo = entrada perfecta
            
            recovery_potential = (distance_to_max / distance_from_min) * 100
            
            self.log(
                f"📈 Análisis Potencial: "
                f"Resistencia +{distance_to_max:.2f}% | "
                f"Soporte -{distance_from_min:.2f}% | "
                f"Potencial: {recovery_potential:.2f}%",
                'info'
            )
    ...
    def _get_market_condition(self):
        ...
        volatility = self.market_history_24h['volatility_24h']
    ...
    def _calculate_buy_scores(self, features, volatility_factor=1.0):
        """
        Calcula scores individuales para cada indicador de COMPRA
        
        ⭐ NUEVO (P0 Phase 7): Ajusta umbrales RSI dinámicamente según volatilidad
        - Volatilidad BAJA → RSI más permisivos (puede ser > 35)
        - Volatilidad ALTA → RSI más estrictos (debe ser < 25)
        """
        scores = {}
        
        # 1. RSI Oversold Score - RECALIBRADO CON VOLATILIDAD DINÁMICA
        rsi = features['rsi']
        # ⭐ Ajustar umbral RSI según volatilidad
        dynamic_rsi_threshold = int(35 * volatility_factor)
        
        if rsi < 20:
            scores['rsi_oversold'] = 100
        elif rsi < 25:
            scores['rsi_oversold'] = 85
        elif rsi < 30:
            scores['rsi_oversold'] = 70
        elif rsi < dynamic_rsi_threshold:  # ⭐ Ahora dinámico
            scores['rsi_oversold'] = 50
            self.log(f"[RSI_DYNAMIC] RSI={rsi:.1f} vs threshold={dynamic_rsi_threshold} (factor={volatility_factor}x)", 'info')
        else:
            scores['rsi_oversold'] = max(0, 40 - rsi)  # Menos penalizacion
        
        # 2. Support Bounce Score
        support_dist = features['support_level']
        if support_dist < 0.03:
            scores['support_bounce'] = 100
        elif support_dist < 0.08:
            scores['support_bounce'] = 80
        elif support_dist < 0.12:
            scores['support_bounce'] = 60
        else:
            scores['support_bounce'] = max(0, 100 - (support_dist * 300))
        
        # 3. Bullish Momentum Score
        momentum = features['momentum']
        if momentum > 0.5:
            scores['bullish_momentum'] = 100
        elif momentum > 0.25:
            scores['bullish_momentum'] = 80
        elif momentum > 0:
            scores['bullish_momentum'] = 60
        else:  # momentum <= 0 (⭐ FIX: NO dar puntos con momentum negativo)
            scores['bullish_momentum'] = 0
        
        # 4. MACD Bullish Score
        macd = features['macd_signal']
        if macd > 0:
            scores['macd_bullish'] = min(100, macd * 200)
        else:
            scores['macd_bullish'] = 0
        ...
        # 6. Bollinger Lower Band Score
        bb_pos = features['bollinger_position']
        if bb_pos < -0.9:
            scores['bollinger_lower'] = 100
        elif bb_pos < -0.7:
            scores['bollinger_lower'] = 80
        elif bb_pos < -0.5:
            scores['bollinger_lower'] = 60
        else:
            scores['bollinger_lower'] = max(0, 40 - (bb_pos * 50))
        
        # 7. Stochastic Oversold Score
        stoch = features['stochastic']
        if stoch < 20:
            scores['stochastic_oversold'] = 100
        elif stoch < 30:
            scores['stochastic_oversold'] = 80
        elif stoch < 40:
            scores['stochastic_oversold'] = 60
        elif stoch < 50:
            scores['stochastic_oversold'] = 30
        else:
            scores['stochastic_oversold'] = 0
        
        # 8. CCI Bullish Score
        cci = features['cci']
        if cci < -100:
            scores['cci_bullish'] = 100
        elif cci < -50:
            scores['cci_bullish'] = 80
        elif cci < 0:
            scores['cci_bullish'] = 50
        elif cci < 100:
            scores['cci_bullish'] = 20
        else:
            scores['cci_bullish'] = 0
        
        # 9. Williams %R Oversold Score
        williams = features['williams_r']
        if williams < -80:
            scores['williams_r_oversold'] = 100
        elif williams < -50:
            scores['williams_r_oversold'] = 80
        elif williams < -20:
            scores['williams_r_oversold'] = 50
        elif williams < 0:
            scores['williams_r_oversold'] = 20
        else:
            scores['williams_r_oversold'] = 0
    ...
    def _calculate_confidence(self, features, scores):
        """⭐ MEJORA #1: Confianza mínima 60% (no 35%). Requiere solidez."""
        ...
        # ⭐ FIX: Elevar mínimos + ADX como multiplicador
        if strong_signals >= 5 and adx >= 25:
            ...
            confidence = 65  # Muchas señales medianas + tendencia fuerte
        elif medium_signals >= 4 and adx >= 20:
            confidence = 62
        elif medium_signals >= 3 and adx >= 20:
            confidence = 60
        else:
            confidence = 50  # ⭐ Elevar de 35 a 50 (mínimo aceptable)
        
        # Penalizar si momentum negativo — proporcional a magnitud (contra dirección BUY)
        momentum = features.get('momentum', 0)
        if momentum < -0.5:
            confidence = max(50, confidence * 0.70)  # -30% conf: caída muy intensa
        elif momentum < -0.25:
            confidence = max(50, confidence * 0.80)  # -20% conf: caída moderada
        elif momentum < 0:
            confidence = max(50, confidence * 0.90)  # -10% conf: caída leve

        return confidence
    
    def _detect_direction_velocity(self, current_rsi, current_volume_sum):
        """
        Detecta microaceleración de señal para entradas tempranas de BUY.
        ...
        try:
            ...
            # BUY: premia aceleración al alza de RSI con confirmación de volumen.
    ...
    def _make_buy_recommendation(self, score, confidence, features):
        ...
        if score >= 85 and confidence >= 75:
            ...
            return 'WEAK_BUY'  # ⭐ Abre con score moderado según microtendencia
        
        return 'HOLD'
        
        return 'HOLD'
    
    def _calculate_macd_bullish(self, closes):
        """Calcula MACD enfocado en señales alcistas"""
        if len(closes) < 26:
            return 0.0
    ...
    def _generate_reasoning(self, features, scores):
        ...
        if scores['rsi_oversold'] >= 80:
            reasons.append(f"RSI en sobreventa ({features['rsi']:.1f})")
        
        if scores['support_bounce'] >= 80:
            reasons.append(f"Cerca de soporte ({features['support_level']:.2f}%)")
        
        if scores['bullish_momentum'] >= 80:
            reasons.append(f"Momentum alcista fuerte ({features['momentum']:.2f}%)")
        
        if scores['macd_bullish'] >= 80:
            reasons.append("MACD cruce alcista")
        
        if scores['bollinger_lower'] >= 80:
            reasons.append("Precio en banda inferior Bollinger")
        
        if scores['stochastic_oversold'] >= 80:
            reasons.append(f"Stochastic en sobreventa ({features['stochastic']:.1f})")
        
        if scores['cci_bullish'] >= 80:
            reasons.append(f"CCI bajista extremo ({features['cci']:.0f})")
        
        if scores['williams_r_oversold'] >= 80:
            reasons.append(f"Williams %R en sobreventa ({features['williams_r']:.1f})")
        
        if scores['adx_trend_strength'] >= 80:
            reasons.append(f"Tendencia alcista fuerte (ADX: {features['adx']:.1f})")
    ...
    def analyze_multi_timeframe(self, symbol='GOLD'):
        """
        Análisis de BUY en múltiples timeframes (M1, M5, M15, M30, H1)
        ...
        Returns:
            dict: {
                ...
                'recommendation': 'STRONG_BUY'|'BUY'|'HOLD'|'WEAK_BUY'
            }
        """
        try:
            ...
            for tf_name in ['M1', 'M5', 'M15', 'M30', 'H1']:
                ...
                volumes = np.array([float(r.get('tick_volume', r[5] if isinstance(r, (list, tuple)) else 0)) if isinstance(r, dict) else float(r[5]) for r in rates])
                
                # Calcular indicadores en este timeframe
                features = {
                    'rsi': self._calculate_rsi(closes),
                    'support_level': self._find_support(lows, closes[-1]),
                    'momentum': self._calculate_momentum(closes),
                    'macd_signal': self._calculate_macd_bullish(closes),
                    'volume_trend': self._calculate_volume_trend(volumes),
                    'bollinger_position': self._calculate_bollinger_position(closes),
                    'trend_strength': self._calculate_trend_strength(closes, 'bullish'),
                ...
                scores = self._calculate_buy_scores(features)
                tf_score = sum(scores[k] * self.weights[k] for k in self.weights.keys())
                ...
                self.log(f"[MTF] {tf_name} BUY Score: {tf_score:.1f}", 'info')
