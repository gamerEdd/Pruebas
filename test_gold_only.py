...
"""
TEST A: GOLD SOLO
Prueba abrir una operación únicamente en GOLD con USE_MULTIPLE_PARTS=False
"""
...
def main():
    ...
    try:
        ...
        if len(silver_positions) > 0:
            log("    ❌ ERROR: Se abrió SILVER pero no debería")
        
        log(f"\n[RESULTADO FINAL]")
        if len(gold_positions) == 1 and len(silver_positions) == 0:
            log("  ✅ TEST A PASADO: Solo GOLD abierto")
            root.destroy()
            mt5.shutdown()
            return 0
        else:
            log(f"  ❌ TEST A FALLIDO: GOLD={len(gold_positions)}, SILVER={len(silver_positions)}")
            root.destroy()
            mt5.shutdown()
            return 1
